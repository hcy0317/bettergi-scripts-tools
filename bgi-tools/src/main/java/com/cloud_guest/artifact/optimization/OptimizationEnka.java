...
public class OptimizationEnka {
    ...
    public OptimizationEnka(ObjectMapper mapper){this.mapper=mapper;}
    ...
    public ObjectNode convert(JsonNode source,JsonNode catalog){
        ...
        for(JsonNode avatar:source.path("avatarInfoList")){
            ...
            var data=matches.get(0);String key=data.path("key").asText();var c=imported.addObject().put("key",key).put("source","enka");
            ...
            var talents=c.putArray("talents");for(String skill:List.of("attack","skill","burst")){
                ...
                if(value.isIntegralNumber())talents.add(value.asInt());else{talents.addNull();warnings.add(key+" 的基础天赋 "+skill+" 未知，请手动补充");}
            }
            ...
            if(!weaponFound){c.putNull("weapon");warnings.add(key+" 武器未知，请手动补充");}
        }
