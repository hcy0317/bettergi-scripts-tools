...
public class OptimizationCommunityService {
    ...
    public ObjectNode search(JsonNode input)throws Exception {
        if(input==null||!input.isObject()||input.toString().length()>8192)throw new IllegalArgumentException("搜索参数格式或大小无效");
        ...
        JsonNode response=fetch("/api/db?q="+URLEncoder.encode(query.toString(),StandardCharsets.UTF_8),
            value->value.isObject()&&(value.isEmpty()||value.path("data").isArray()));
        // The maintained database serializes an empty search as {}, not data: [].
        JsonNode data=response.isObject()&&response.isEmpty()?mapper.createArrayNode():response.path("data");
        var result=mapper.createObjectNode().put("page",page).put("hasMore",data.size()==20);var entries=result.putArray("entries");
    ...
    public ObjectNode read(String reference)throws Exception {
        ...
        if(!validId(id))throw new IllegalArgumentException("社区条目编号无效");
        String expectedId=id;
        JsonNode entry=fetch("/api/db/id/"+id,value->value.isObject()&&value.path("_id").asText().equals(expectedId)
            &&value.path("config").isTextual()&&value.path("config").asText().length()<=100_000);
    ...
    private JsonNode fetch(String path,Predicate<JsonNode> valid)throws Exception {
        ...
        if(!slots.tryAcquire())throw new OptimizationCommunityException(429,"社区请求正在处理中，请稍后重试");
        try{
            ...
            if(response.statusCode()==429)throw new OptimizationCommunityException(429,"社区服务限流，请稍后重试，或直接粘贴脚本");
            if(response.statusCode()==404&&path.startsWith("/api/db/id/"))throw new OptimizationCommunityException(404,"未找到该社区条目，请检查链接或编号，也可直接粘贴脚本");
            if(response.statusCode()!=200)throw new OptimizationCommunityException(502,"社区服务返回 HTTP "+response.statusCode()+"，请稍后重试，或直接粘贴脚本");
            JsonNode value=mapper.readTree(response.body());
            if(value==null||!valid.test(value))throw new OptimizationCommunityException(502,"社区响应格式变化或条目内容不完整，请稍后重试，或直接粘贴脚本；未修改任何档案");
            ...
            throw new OptimizationCommunityException(504,"社区请求超时，请稍后重试，或直接粘贴脚本",error);
        }catch(com.fasterxml.jackson.core.JsonProcessingException error){
            throw new OptimizationCommunityException(502,"社区返回的内容不是有效 JSON，请稍后重试，或直接粘贴脚本",error);
        }catch(java.io.IOException error){
            for(Throwable cause=error;cause!=null;cause=cause.getCause())if(cause instanceof OptimizationCommunityException bounded)throw bounded;
            throw new OptimizationCommunityException(503,"社区连接失败，请稍后重试，或直接粘贴脚本",error);
        }catch(InterruptedException error){
            Thread.currentThread().interrupt();
            throw new OptimizationCommunityException(503,"社区请求已中止，请重试",error);
    ...
    private static final class BoundedBody implements HttpResponse.BodySubscriber<byte[]> {
        ...
        public void onNext(List<ByteBuffer> buffers){for(ByteBuffer buffer:buffers){if(bytes.size()+buffer.remaining()>limit){subscription.cancel();completed.completeExceptionally(new OptimizationCommunityException(502,"社区响应超过大小限制，请缩小筛选或直接粘贴脚本"));return;}byte[] chunk=new byte[buffer.remaining()];buffer.get(chunk);bytes.writeBytes(chunk);}}
