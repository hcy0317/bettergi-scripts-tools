...
class OptimizerBrowserHarnessTest {
    @Test void serveIsolatedWorkbenchForBrowserVerification() throws Exception {
        ...
        for(int n=0;n<2;n++)for(int s=0;s<5;s++)items.add(new ArtifactItem(n*5+s,"EmblemOfSeveredFate",slots[s],20,5,keys[s],List.of(new ArtifactSubstat("critDMG_",14+7*n,false)),n==0?"Amber":"Kaeya",false));
        ...
        var workspace=new OptimizationWorkspace(store,mapper);var gateway=new GcsimGateway(mapper,null,exe);var mainstats=new OptimizationMainStats(mapper);var jobs=new OptimizationJobs(store,scans,workspace,mainstats,gateway,mapper);
