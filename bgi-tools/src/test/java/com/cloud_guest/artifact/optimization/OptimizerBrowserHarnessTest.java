...
import com.cloud_guest.controller.ArtifactCommunityController;
...
class OptimizerBrowserHarnessTest {
    ...
    @Test void serveIsolatedWorkbenchForBrowserVerification() throws Exception {
        ...
        Path fixtureRoot=Files.createDirectories(launchRoot.resolve("fixture-install"));Path aliases=fixtureRoot.resolve("GameTask/AutoFight/Assets/combat_avatar.json");Files.createDirectories(aliases.getParent());
        Files.copy(Path.of(System.getProperty("artifact.optimizer.test.bettergiRoot")).resolve("GameTask/AutoFight/Assets/combat_avatar.json"),aliases);
        Path strategy=fixtureRoot.resolve("User/AutoFight/合成示例/基础.txt");Files.createDirectories(strategy.getParent());Files.writeString(strategy,"安柏 e,attack(2)\n凯亚 q(required)");
        when(sources.betterGiRoot()).thenReturn(fixtureRoot);
        ...
        var community=new ArtifactCommunityController(new OptimizationCommunityService(mapper),mapper);
        ...
        server.createContext("/",exchange->{
            try {
                ...
                else if(path.equals(prefix+"/rotations/strategies/source"))result=rotations.source(params.get("name"));
                else if(path.equals(prefix+"/community/search"))result=community.search((ObjectNode)mapper.readTree(exchange.getRequestBody()));
                else if(path.equals(prefix+"/community/preview"))result=community.preview((ObjectNode)mapper.readTree(exchange.getRequestBody()));
                else if(path.equals(prefix+"/community/outline"))result=community.outline((ObjectNode)mapper.readTree(exchange.getRequestBody()));
            ...
            }catch(Exception error){boolean validation=error instanceof OptimizationValidationException;var value=new LinkedHashMap<String,Object>();value.put("code",validation?400:500);value.put("message",Objects.toString(error.getMessage(),"error"));if(validation)value.put("data",((OptimizationValidationException)error).issues());byte[] bytes=mapper.writeValueAsBytes(value);exchange.getResponseHeaders().set("Content-Type","application/json");exchange.sendResponseHeaders(validation?400:200,bytes.length);exchange.getResponseBody().write(bytes);}finally{exchange.close();}
        });
