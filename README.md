...
`CD-Aware-AutoGather` 和 `FullyAutoAndSemiAutoTools`，后续模块通过统一适配接口增加或替换。确认新账本、
网页修改模块配置、BetterGI 修改专属组配置以及权威库存回写都会自动同步同一 UID 的专属组；人工设置按
最后修改时间收敛，账本派生的秘境、Boss、地方特产和怪物路线则始终按最新缺口重新生成。

执行投影不把缺口机械换算为固定总次数。秘境/Boss 每个安全批次及组末地方特产/怪物库存复核都通过
同 UID/revision 租约和稳定幂等键回写；任一未知库存会进入等待重新对账，不继续消耗资源。
