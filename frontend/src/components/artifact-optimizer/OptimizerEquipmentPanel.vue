...
import {validateArtifactLaunch} from '@/features/artifact-analysis/model.js'
import ArtifactLaunchDialog from '@/components/artifact/ArtifactLaunchDialog.vue'
...
function clear(){generation++;if(timer)clearTimeout(timer);timer=null;plan.value=null;dialog.value=false;launchDialog.value=false;busy.value=false}
watch(()=>[props.uid,props.job.id],clear)
async function poll(id,g,uid){if(timer)clearTimeout(timer);timer=null;try{const value=await api.getEquipment(uid,id);if(g!==generation||plan.value?.id!==id)return;plan.value=value;if(value.state!=='CONFIRMED')launchDialog.value=false;if(['CONFIRMED','RUNNING'].includes(value.state))timer=setTimeout(()=>poll(id,g,uid),1500)}catch(e){if(g===generation)error.value=e.message||String(e)}}
...
async function confirm(){if(!accepted.value)return;const g=generation;busy.value=true;error.value='';try{const value=await api.confirmEquipment(props.uid,plan.value.id,plan.value.digest);if(g!==generation)return;plan.value=value;dialog.value=false;if(!validateArtifactLaunch(value.launch,'EXECUTE_EQUIP_PLAN'))throw new Error('穿戴请求的连接地址无效，请取消后重新预览');launchDialog.value=true;await poll(value.id,g,props.uid)}catch(e){if(g===generation)error.value=e.message||String(e)}finally{if(g===generation)busy.value=false}}
...
<template>
  <section class="equipment-panel">
    ...
    <template v-if="plan"><h4>{{ states[plan.state]||plan.state }}</h4><p v-if="plan.state==='CONFIRMED'">已生成一次性穿戴请求。点击连接后由 BetterGI 接手；网页不会自行启动程序。</p><el-button v-if="plan.state==='CONFIRMED'&&validateArtifactLaunch(plan.launch,'EXECUTE_EQUIP_PLAN')" type="primary" @click="launchDialog=true">连接 BetterGI</el-button><el-button v-if="['CONFIRMED','RUNNING'].includes(plan.state)" type="danger" plain @click="cancel">请求安全停止</el-button><el-button @click="poll(plan.id,generation,uid)">读取执行状态</el-button>
    ...
    <ArtifactLaunchDialog v-if="plan?.state==='CONFIRMED'" v-model:open="launchDialog" :launch="plan.launch" task-label="执行已确认的圣遗物穿戴" @launched="poll(plan.id,generation,uid)"/>
  </section>
</template>
