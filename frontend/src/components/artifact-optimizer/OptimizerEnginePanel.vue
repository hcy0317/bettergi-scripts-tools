...
async function install(){try{await ElMessageBox.confirm('从本仓库发布包验证并切换引擎？已有计算结束前不会切换。旧有效程序和个人档案保留，手动 增益 需要针对新版本复核。','确认更新',{type:'warning'})}catch{return}busy.value=true;error.value='';try{const value=await api.installEnginePackage(assetId.value);message.value=value.note;emit('changed')}catch(e){error.value=e.message||String(e)}finally{busy.value=false}}
async function rollback(){try{await ElMessageBox.confirm('回到上一有效引擎及其数据？不会覆盖个人 方案 或手工补充。','确认回退',{type:'warning'})}catch{return}busy.value=true;error.value='';try{const value=await api.rollbackEngine();message.value=value.note;emit('changed')}catch(e){error.value=e.message||String(e)}finally{busy.value=false}}
</script>

<template>
  <section class="engine-panel"><h2>gcsim 引擎与持续维护</h2><p>角色、武器、套装、技能数值和动态反应来自同一个固定版本的 gcsim。社区主词条等级表是独立补充；个人条件与 增益 不随更新覆盖。</p>
