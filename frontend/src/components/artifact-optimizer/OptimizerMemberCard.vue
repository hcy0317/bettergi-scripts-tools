...
<template>
  <article :class="['member-card',{participating:selected}]" :aria-label="`${profileLabel(catalog,profile)}队员卡片`">
    ...
    <el-form v-else label-position="top" class="override-fields">
      <el-form-item label="角色等级"><el-input-number v-model="member.profile.level" :min="1" :max="100"/></el-form-item>
      <el-form-item label="突破等级上限"><el-select v-model="member.profile.maxLevel"><el-option v-for="n in [20,40,50,60,70,80,90,100]" :key="n" :value="n" :label="String(n)"/></el-select></el-form-item>
      <el-form-item label="命座"><el-input-number v-model="member.profile.constellation" :min="0" :max="6"/></el-form-item>
      <el-form-item label="武器"><el-select v-model="member.profile.weapon" filterable><el-option v-for="w in weapons" :key="w.key" :value="w.key" :label="weaponLabel(catalog,w.key)"/></el-select></el-form-item>
      <el-form-item label="武器等级"><el-input-number v-model="member.profile.weaponLevel" :min="1" :max="90"/></el-form-item>
      <el-form-item label="武器突破上限"><el-select v-model="member.profile.weaponMaxLevel"><el-option v-for="n in [20,40,50,60,70,80,90]" :key="n" :value="n" :label="String(n)"/></el-select></el-form-item>
      <el-form-item label="精炼"><el-input-number v-model="member.profile.refinement" :min="1" :max="5"/></el-form-item>
    ...
    <el-button text type="danger" @click="emit('remove')">移除队员</el-button>
  </article>
</template>
<style scoped>
