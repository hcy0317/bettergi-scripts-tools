...
const props=defineProps({job:{type:Object,default:null},items:{type:Array,default:()=>[]},characters:{type:Array,default:()=>[]}})
const result=computed(()=>props.job?.result)
const plan=computed(()=>result.value?.plan)
const characterName=key=>props.characters.find(c=>c.key===key)?.name||key
...
<template>
  <section aria-live="polite">
    <el-empty v-if="!job" description="选择角色、Build 和背包快照后开始计算。每名参选角色都会得到一套通用装备。"/>
    <template v-else>
      ...
      <template v-if="result">
        <div class="summary"><span>模拟评估 <strong>{{ result.evaluations }}</strong></span><span>搜索范围 <strong>{{ result.exhaustive?'本次完整枚举':'有预算搜索' }}</strong></span><span v-if="plan">加权实际总 DPS <strong>{{ metric(plan.rank.weightedDps) }}</strong></span></div>
        ...
        <template v-if="plan">
          ...
          <EquipmentPanel :uid="job.uid" :job="job"/>
        </template>
      </template>
    </template>
  </section>
