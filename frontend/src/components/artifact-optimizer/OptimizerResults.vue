...
import {characterLabel,profileLabel,setLabel,jobLabels,metricLabels,buildLabel} from '@/features/artifact-optimizer/localization.js'
...
<template>
  <section aria-live="polite">
    <el-empty v-if="!job" description="选择角色、方案和背包快照后开始计算。每名参选角色都会得到一套通用装备。"/>
    <template v-else>
      ...
      <el-alert v-if="scenes.some(scene=>scene.support==='trial')" title="带假设试算：此方案仅通过声明的模拟样本，不代表实机已验证。具体映射和假设保留在下方报告中。" type="warning" :closable="false"/>
      ...
      <template v-if="result">
        <div class="summary"><span>模拟评估 <strong>{{ result.evaluations }}</strong></span><span>搜索范围 <strong>{{ result.exhaustive?'本次完整枚举':'有预算搜索' }}</strong></span><span v-if="plan">加权总每秒伤害 <strong>{{ metric(plan.rank.weightedDps) }}</strong></span></div>
        ...
        <template v-if="!plan"><el-alert v-for="issue in result.issues||[]" :key="issue" :title="issue" type="warning" :closable="false"/></template>
        <template v-if="plan">
          <p class="hint">所有已选方案共用以下装备，实物不会被两名角色重复占用。验证仅覆盖本次声明样本，不代表实机已验证。</p>
