...
import {applyCommunityPreview,describeCommunitySettings} from '@/features/artifact-optimizer/community.js'
...
let requestId=0,controller=null,binding=null
const mismatches=computed(()=>(preview.value?.characters||[]).filter(key=>!props.build.members.some(m=>m.character===key)))
function begin(){controller?.abort();controller=new AbortController();searching.value=false;reading.value=false;error.value='';return ++requestId}
function show(){open.value=true;include.value=props.build.members.map(m=>m.character);preview.value=null;search(1)}
async function search(next=1){const id=begin();searching.value=true;try{const value=await api.searchCommunity({include:include.value,exclude:exclude.value,tags:tags.value,excludedTags:excludedTags.value,sort:sort.value,direction:'desc',page:next},controller.signal);if(id!==requestId)return;entries.value=value.entries;hasMore.value=value.hasMore;page.value=next}catch(e){if(id===requestId&&!controller.signal.aborted)error.value=e.message||'社区搜索失败，请稍后重试'}finally{if(id===requestId)searching.value=false}}
async function read(input){const id=begin();reading.value=true;const current={uid:props.uid,id:props.build.id,snapshot:JSON.stringify(props.build)};try{const value=await api.previewCommunity(input,controller.signal);if(id!==requestId)return;preview.value=value;binding=current;applyScript.value=true;applySettings.value=true;applySampling.value=true;previewOpen.value=true}catch(e){if(id===requestId&&!controller.signal.aborted)error.value=e.message||'脚本预览失败'}finally{if(id===requestId)reading.value=false}}
function apply(){error.value='';if(!binding||binding.uid!==props.uid||binding.id!==props.build.id||binding.snapshot!==JSON.stringify(props.build)){error.value='方案在预览后发生了修改，请重新预览，避免覆盖新编辑';return}try{applyCommunityPreview(props.build,preview.value,{script:applyScript.value,settings:applySettings.value});if(applySampling.value&&preview.value.validationSamples)emit('sampling',preview.value.validationSamples);previewOpen.value=false;open.value=false}catch(e){error.value=e.message}}
watch(()=>[props.uid,props.build.id],()=>{begin();preview.value=null;previewOpen.value=false;open.value=false})
...
<template>
  <section class="community-entry"><el-button type="primary" plain @click="show">搜索社区循环并导入</el-button><span>按队员筛选 sim pact / gcsim 公开脚本，或粘贴链接与原文。</span>
    <el-dialog v-model="open" title="社区循环搜索与快速导入" width="min(1180px,96vw)">
      <el-alert v-if="error" :title="error" type="error" :closable="false"/>
      <el-form label-position="top" class="filters">
        <el-form-item label="包含角色（最多四人）"><el-select v-model="include" multiple filterable :multiple-limit="4"><el-option v-for="c in catalog.characters||[]" :key="c.key" :value="c.key" :label="characterLabel(catalog,c.key)"/></el-select></el-form-item>
        <el-form-item label="排除角色"><el-select v-model="exclude" multiple filterable :multiple-limit="16"><el-option v-for="c in catalog.characters||[]" :key="c.key" :value="c.key" :label="characterLabel(catalog,c.key)"/></el-select></el-form-item>
        ...
        <el-form-item label="应用筛选"><el-button type="primary" :loading="searching" @click="search(1)">搜索</el-button></el-form-item>
      </el-form>
      <p class="hint">只发送角色与标签筛选，不发送账号、背包或个人培养数据。参考伤害来自作者条件，不能当作你的配装收益。</p>
      <el-skeleton v-if="searching" :rows="5" animated/>
      <el-empty v-else-if="!error&&!entries.length" description="没有匹配的公开方案，可减少包含角色或调整标签。"/>
      <div v-else class="community-results"><article v-for="entry in entries" :key="entry.id"><div class="entry-team"><strong>{{ (entry.characters||[]).map(key=>characterLabel(catalog,key)).join(' / ') }}</strong><span>每目标每秒伤害 {{ metric(entry.dps) }}</span></div><p>{{ entry.description||'作者未提供说明' }}</p><div class="entry-actions"><a :href="entry.url" target="_blank" rel="noopener noreferrer">查看原始条目</a><el-button :loading="reading" @click="read({reference:entry.id})">预览导入</el-button></div></article></div>
      <div class="pagination"><el-button :disabled="page===1||searching" @click="search(page-1)">上一页</el-button><span>第 {{ page }} 页</span><el-button :disabled="!hasMore||searching" @click="search(page+1)">下一页</el-button></div>
    ...
    <el-dialog v-model="previewOpen" title="确认循环与场景导入" width="min(1000px,96vw)" append-to-body>
      ...
      </template><template #footer><el-button @click="previewOpen=false">暂不导入</el-button><el-button type="primary" :disabled="!preview?.importable||(!applyScript&&!applySettings&&!applySampling)" @click="apply">确认应用到当前方案</el-button></template>
    </el-dialog>
  </section>
</template>
<style scoped>
