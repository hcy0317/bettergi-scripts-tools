<script setup>
import {ref} from 'vue'
import {characterLabel,profileLabel,weaponLabel,statLabel,elementLabels} from '@/features/artifact-optimizer/localization.js'
...
<template>
  <section class="build-editor">
    <header><h2>配队 方案</h2><el-button type="danger" plain @click="emit('remove')">删除 方案</el-button></header>
    ...
    <div v-for="(member,index) in build.members" :key="member.character" class="member">
      ...
      <el-checkbox :model-value="Boolean(member.profile)" @change="on=>toggleOverride(member,on)">此 方案 单独设置等级、武器、命座和天赋</el-checkbox>
    ...
    <el-collapse-item title="通用 增益 补充与标记试算" name="buffs">
      <el-alert title="手动 增益 是明确假设，不是实机已触发的证据。无法安全替代原生效果时只允许额外或待复核，未知机制不会自动变成可用。" type="warning" :closable="false"/>
      <el-button class="add-buff" @click="addBuff">添加通用 增益</el-button>
      <el-select model-value="" placeholder="复用已保存的 增益 模板" @change="useTemplate"><el-option v-for="b in templates" :key="b.id" :value="b.id" :label="`${b.source||b.kind} ${b.stat?statLabel(b.stat):b.element?elementLabels[b.element]:({normal:'普通攻击',skill:'元素战技',burst:'元素爆发'}[b.attackTag]||'')} ${b.value}`"/></el-select>
      <el-form v-for="(buff,i) in build.buffs" :key="buff.id" label-position="top" class="buff-form">
        <el-form-item label="此 方案 中启用"><el-switch :model-value="buff.enabled!==false" @update:model-value="value=>buff.enabled=value"/></el-form-item><el-form-item label="保存为可复用模板"><el-button @click="emit('save-buff-template',{...JSON.parse(JSON.stringify(buff)),id:crypto.randomUUID()})">保存模板</el-button></el-form-item>
        ...
        <el-form-item label="持续帧数（60 帧 = 1 秒，-1 常驻）"><el-input-number v-model="buff.durationFrames" :min="-1" :max="36000"/></el-form-item><el-form-item label="与原生效果关系"><el-select v-model="buff.relationship" @change="()=>buff.reviewedEngineRevision=catalog.engineRevision"><el-option value="pending_review" label="可能重叠，待复核"/><el-option value="additional" label="确认是额外效果"/></el-select></el-form-item><el-form-item label="来源说明"><el-input v-model="buff.source"/></el-form-item><el-button text type="danger" @click="build.buffs.splice(i,1)">删除此 增益</el-button>
      </el-form>
