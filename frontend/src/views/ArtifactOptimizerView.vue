...
const names=Object.fromEntries(Object.entries(artifactCharacterLabels).map(([key,label])=>[key.toLowerCase().replace(/[_ -]/g,''),label]))
...
const filteredCharacters=computed(()=>workspace.value.characters.filter(c=>[c.name,c.key,...(c.tags||[])].join(' ').toLowerCase().includes(search.value.toLowerCase())))
...
function addCharacter(){if(!addKey.value||workspace.value.characters.some(c=>c.key===addKey.value))return;const c=newCharacter(addKey.value,names[addKey.value]||addKey.value);workspace.value.characters.push(c);activeCharacter.value=c.key;addDialog.value=false;addKey.value=''}
...
<template>
  <main class="optimizer-page feature-page-background">
    <section class="optimizer-shell">
      <header class="page-header"><div class="nav"><el-button circle :icon="ArrowLeft" aria-label="返回" @click="router.back()"/><el-button circle :icon="HomeFilled" aria-label="首页" @click="router.push('/')"/></div><div><h1>圣遗物自动配装</h1><p>同一背包，多人分配。一套装备兼顾多个配队 Build。</p></div><el-tag v-if="dirty" type="warning">有未保存编辑</el-tag></header>
        ...
        <section v-if="tab==='characters'" class="workbench">
          <aside class="roster"><el-input v-model="search" clearable placeholder="搜索角色或 TAG" aria-label="搜索角色"/><el-button :icon="Plus" @click="addDialog=true">添加角色</el-button><div v-if="!workspace.characters.length" class="hint">添加个人档案，或从公开 Enka 展柜导入。</div>
            <div v-for="c in filteredCharacters" :key="c.key" :class="['role-row',{active:c.key===activeCharacter}]">
              <el-checkbox :model-value="selected.includes(c.key)" :aria-label="`选择${c.name||c.key}参与计算`" @change="checked=>selected=checked?[...selected,c.key]:selected.filter(k=>k!==c.key)"/>
              <button type="button" class="role-select" @click="activeCharacter=c.key"><img v-if="catalog.characters?.find(m=>m.key===c.key)?.icon_name" :src="`https://enka.network/ui/${catalog.characters.find(m=>m.key===c.key).icon_name}.png`" alt="" loading="lazy"/><span><strong>{{ c.name||c.key }}</strong><small>{{ c.builds?.length||0 }} 个 Build{{ c.protected?' · 装备保护':'' }}</small><small v-if="c.tags?.length">{{ c.tags.join(' / ') }}</small></span></button>
            </div>
          </aside>
