...
- 生成专属组时只加入当前缺口实际需要且已启用的模块；周本未确认风险条款时不得进入脚本组。
...
- 自动体力计划保留模块总开关，并将天赋书秘境、摩拉地脉和大英雄经验地脉拆为三个独立启停开关；关闭的类型不得写入下一轮 AutoPlan 行动，AutoPlan JS 任务名也只列出实际启用的内容。
...
- 已能生成并启动普通秘境、地脉、世界 Boss、周本、地方特产和怪物路线的 UID 专属单轮执行组，但尚未建立执行租约、奖励事件、战前/战后权威背包回写或完成门禁。
- `RunCountInventoryItemTask`、奖励明细回写、合成闭环和自动重新发放下一行动仍按本文件 P1-P4 接入；当前启动完成不等于需求完成。
